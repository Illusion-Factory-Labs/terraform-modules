variable "azure_kubernetes_clusters" {
  ...
  validation {
    condition = alltrue(
      [
        for _, aks in var.azure_kubernetes_clusters : aks.dns_prefix != null || aks.dns_prefix_private_cluster != null
      ]
    )
    error_message = <<EOT
        Para cada cluster, ao menos uma das propriedades, 'dns_prefix' ou 'dns_prefix_private_cluster', deve ser definida.
    EOT
  }
  validation {
    condition = alltrue(
      [
        for _, aks in var.azure_kubernetes_clusters : can(regex("^[A-Za-z0-9](?:[A-Za-z0-9-]{0,52}[A-Za-z0-9])?$", aks.dns_prefix))
      ]
    )
    error_message = <<EOT
